...
C-----------------------------------------------------------------------
 ...
 999  END
      ...
      END
c
      function setbrtop(td,nt)
      implicit none 
      double precision setbrtop
      integer td(4),nt
c
      integer fnstate(4),td0(4),ord(4),j1,j2,j3,j4,j5,j6,ngood
      double precision td1(4)
      logical fns
c
      do j1= 1,4
         td1(j1)= float(td(j1))
                  ...
                  do j4= 1,9
                     ...
                     do j5= 1,4
                        if(td0(j5).le.3) then
                           ...
                           fns= .false.
 10                        continue
                        elseif(td0(j5).eq.4) then
                           do j6= 1,4
                              if(fnstate(j6).le.3) then
                                 fnstate(j6)= 1000
                                 goto 11
                              endif
                           enddo
                           fns= .false.
 11                        continue
                        elseif(td0(j5).eq.5) then
                           do j6= 1,4
                              if(fnstate(j6).ge.4.and.
     +                           fnstate(j6).le.9) then
                                 fnstate(j6)= 1000
                                 goto 12
                              endif
                           enddo
                           fns= .false.
 12                        continue
                        elseif(td0(j5).eq.6) then
c                          always accepted
                        else
                           write(*,*) 'something wrong in setbrtop'
                        endif
                     enddo
                     if(fns) ngood= ngood+1
                  enddo
               enddo
            enddo
         enddo
         setbrtop= float(ngood)/9.d0/9.d0/9.d0/9.d0
      elseif(nt.eq.2) then
         do j1=1,9
            do j2=1,9
               fnstate(1)= j1
...
c                    always accepted
            ...
            enddo
         enddo
         setbrtop= float(ngood)/9.d0/9.d0
      elseif(nt.eq.1) then
         do j1=1,9
            fnstate(1)= j1
            fns= .true.
...
c     always accepted
            ...
            if(fns) ngood= ngood+1
         enddo
         setbrtop= float(ngood)/9.d0
      else
         write(*,*) 'wrong nt in setbrtop'
         stop
      endif
c     
      return
