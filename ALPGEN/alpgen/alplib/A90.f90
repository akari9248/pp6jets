...
module incnst
!
  integer*4, parameter :: nmax=20     !maximum number of particles
!  integer*4, parameter :: nxop=10    !maximum number of particles, optimized version
  integer*4, parameter :: nlor=20     !maximum number of lorentz degrees of freedom
  integer*4, parameter :: npmax=47    !maximum number of different particles "flavour"
  integer*4, parameter :: nmrgmx=5001 !
!  integer*4, parameter :: nprmx=2**nmax+10
!
end module incnst
!
module extsrc
...
!    integer :: nprc                          !labelling process number 
...
!
  use incnst
  use alptyp
! particle naming scheme internal to alpha   NB fermionbars before fermions
  integer*4, parameter :: gluon=1,wm=4,wp=3,z=2,photon=5,qdbr=6,qbtbr=7,qupbrl=8,qdbrl=9,     &
                          ...
                          ebrr=17,qsbrl=18,qsbrr=19,qtopbr=20,qupbr=21,qsbr=22,qd=23,qbt=24,   &
                          qupl=25,qdl=26,qupr=27,qdr=28,el=29,nue=30,qc=31,qcr=32,er=33,      &
                          qsl=34,qsr=35,qtop=36,qcl=37,qup=38,qs=39,zzww=40,higgs=41,        &
                          xglu=42,yglu=43,qcdagh=44,qcdgh=45,xpp=46,ypp=47
...
!
  real*8 :: gauge                ! gauge fixing parameter
  real*8 :: ggh                  ! effective higgs-glu-glu coupling
!
  real*8 :: hww_coup=1.d0,hzz_coup=1.d0 
!
  logical :: gaugedefault=.true.
!
...
! NEXC,NEXCSM arrays for the iteration
  integer*4, parameter :: nexcmax=20
...
!
  call fillpr(gluon,0,glu,10,3,-99,prtcl(gluon)); 
  call fillpr(photon,0,pht,5,1,-99,prtcl(photon)); 
  ...
  call fillpr(qcdgh,37,scal,1,3,-99,prtcl(qcdagh));
  call fillpr(qcdagh,38,scal,1,3,-99,prtcl(qcdgh));
  call fillpr(xglu,0,xgl,3,3,-99,prtcl(yglu));
  call fillpr(yglu,0,xgl,3,3,-99,prtcl(xglu));
  call fillpr(qsbr,39,frm,4,2,dn,prtcl(qs));
  call fillpr(qs,40,frmbr,4,2,dn,prtcl(qsbr));
!
...
!
  call inital
!
  if(gaugedefault) gauge=1.d0
  if(masses(higgs).lt.1.d-10) masses(higgs)=1.d-8  ! to prevent 1/0
!
  sqd2=sqrt(2.d0)
...
!
  oper(gluon,qcdagh,qcdgh)=42; oper(qcdgh,gluon,qcdagh)=43; oper(qcdagh,gluon,qcdgh)=44; 
  coup(gluon,qcdagh,qcdgh)=-gstrong/sqd2; coup(qcdgh,gluon,qcdagh)=-gstrong/sqd2; 
  coup(qcdagh,gluon,qcdgh)=-gstrong/sqd2
!
...
!
  ...
  oper(wp,wm,higgs)=38; coup(wp,wm,higgs)=masses(wp)*g2weak/sqd2*hww_coup; oper(wm,wp,higgs)=38; 
  coup(wm,wp,higgs)=coup(wp,wm,higgs); oper(higgs,wp,wm)=37; coup(higgs,wp,wm)=coup(wp,wm,higgs); 
!
  oper(z,z,higgs)=38; coup(z,z,higgs)=masses(z)*gbar/sqd2*hzz_coup;
  oper(higgs,z,z)=37; coup(higgs,z,z)=coup(z,z,higgs);
!
...
!
  oper(z,photon,zzww)=28; oper(photon,z,zzww)=28; oper(zzww,z,photon)=29; 
  coup(z,photon,zzww)=g2weak/sqd2*ctw*stw; coup(zzww,z,photon)=g2weak/sqd2*ctw*stw; 
  coup(photon,z,zzww)=g2weak/sqd2*ctw*stw; 
!!$  coup(z,photon,zzww)=-g2weak/sqd2*ctw*stw; coup(zzww,z,photon)=-g2weak/sqd2*ctw*stw; 
!!$  coup(photon,z,zzww)=-g2weak/sqd2*ctw*stw;   !old
!
  oper(photon,photon,zzww)=28;  oper(zzww,photon,photon)=29; 
...
!
  ...
  oper(photon,wp,wm)=30;  oper(wp,wm,photon)=30; oper(wm,wp,photon)=30; 
  coup(photon,wp,wm)=g2weak*stw/sqd2; coup(wp,wm,photon)=g2weak*stw/sqd2; coup(wm,wp,photon)=-g2weak*stw/sqd2; 
!!$  coup(photon,wp,wm)=-g2weak*stw/sqd2; coup(wp,wm,photon)=-g2weak*stw/sqd2; coup(wm,wp,photon)=g2weak*stw/sqd2; 
  qrtc=0.d0;
  qrtc(1,1)=-1.d0/ctw; qrtc(2,2)=-1.d0/ctw; qrtc(2,3)=1.d0/ctw;  !Z5 W+ W-
...
!
  nt=6; tmp1(1:nt)=(/qupbr,qdbr,qcbr,qbtbr,qtopbr,qsbr/); tmp2(1:nt)=(/qup,qd,qc,qbt,qtop,qs/); 
  do j1=1,nt  
...
!
  ...
  do j1=1,nt  
    oper(gluon,tmp1(j1),tmp2(j1))=13; oper(tmp2(j1),gluon,tmp1(j1))=14
    oper(tmp1(j1),gluon,tmp2(j1))=15; coup(gluon,tmp1(j1),tmp2(j1))=gstrong/sqd2; 
    coup(tmp2(j1),gluon,tmp1(j1))=gstrong/sqd2; coup(tmp1(j1),gluon,tmp2(j1))=gstrong/sqd2; 
  enddo
!
  nt=4; tmp1(1:nt)=(/qupbrr,qdbrr,qcbrr,qsbrr/); tmp2(1:nt)=(/qupr,qdr,qcr,qsr/); 
  do j1=1,nt  
    oper(gluon,tmp1(j1),tmp2(j1))=19; oper(tmp2(j1),gluon,tmp1(j1))=20;
    oper(tmp1(j1),gluon,tmp2(j1))=21; coup(gluon,tmp1(j1),tmp2(j1))=gstrong/sqd2; 
    coup(tmp2(j1),gluon,tmp1(j1))=gstrong/sqd2; coup(tmp1(j1),gluon,tmp2(j1))=gstrong/sqd2; 
  enddo
!
  nt=3; tmp1(1:nt)=(/qdbr,qbtbr,qsbr/); tmp2(1:nt)=(/qup,qtop,qc/); 
  do j1=1,nt  
  ...
  enddo
!
  nt=3; tmp1(1:nt)=(/qupbr,qtopbr,qcbr/); tmp2(1:nt)=(/qd,qbt,qs/); 
  do j1=1,nt  
...
!
  ...
  enddo
!
  nt=6; tmp1(1:nt)=(/qupbr,qdbr,qbtbr,qtopbr,qcbr,qsbr/); tmp2(1:nt)=(/qup,qd,qbt,qtop,qc,qs/); 
  do j1=1,nt 
...
!
  nt=6; tmp1(1:nt)=(/qupbr,qdbr,qcbr,qbtbr,qtopbr,qsbr/); tmp2(1:nt)=(/qup,qd,qc,qbt,qtop,qs/); 
  ch(1:nt)=-(/2.d0/3.d0*emch,-1.d0/3.d0*emch,2.d0/3.d0*emch,-1.d0/3.d0*emch,2.d0/3.d0*emch, -1.d0/3.d0*emch/)
  do j1=1,nt  
...
!
  ...
  do j1=1,nt  
    oper(photon,tmp1(j1),tmp2(j1))=13; oper(tmp2(j1),photon,tmp1(j1))=14;
    oper(tmp1(j1),photon,tmp2(j1))=15; coup(photon,tmp1(j1),tmp2(j1))=ch(j1); 
    coup(tmp2(j1),photon,tmp1(j1))=ch(j1); coup(tmp1(j1),photon,tmp2(j1))=ch(j1);
  enddo
!
  ...
  do j1=1,nt  
    oper(photon,tmp1(j1),tmp2(j1))=19; oper(tmp2(j1),photon,tmp1(j1))=20;
    oper(tmp1(j1),photon,tmp2(j1))=21; coup(photon,tmp1(j1),tmp2(j1))=ch(j1); 
    coup(tmp2(j1),photon,tmp1(j1))=ch(j1); coup(tmp1(j1),photon,tmp2(j1))=ch(j1);
  enddo
!
end subroutine initcoup
!
subroutine setgauge (gauge0)
!
  use couplings, only : gauge,gaugedefault
  implicit none
  real*8, intent(in) :: gauge0
  gaugedefault=.false.
  gauge = gauge0
!
end subroutine setgauge
!
module utilities
!
  implicit none
...
!
! Input   : PRCSS%FLV(NMAX) particle flavours, according to alpha Naming scheme
!           PRCSS%MOM(4*NMAX) particle momenta (E,px,py,pz)
!           PRCSS%NPRC process label (to deal with many process simultaneously)
!
! Output  : MTEL matrix element
!
  use alptyp
  use strfld
  use prcint
  use couplings, only : oper, coup, masses
  use incol
  use dimen
  use utilities
  use running
...
!
            ...
            if (padd.eq.0) then             !new configuration
              ...
              do j5=1,nc
                ...
                if((poscol+nca.gt.dim(d_offshcol).or.posfld+nl1.gt.dim(d_offsh))) then
                  call raiseexception1('iter',nprt,poscol+nca,posfld+nl1,prcss%col,prcss%flv)
                  mtel=0.d0
                  return
                endif
!
...
!
  if (fststp.eq.1.and.dbg) then
    ...
    if (poscol.gt.pscit) then
      ...
      write(*,*)'color string', prcss%col(1:2*nprt)
    endif
...
!
              ...
              if (padd.eq.0) then             !new configuration
                ...
                do j5=1,nc
                  if(poscol+nca.gt.dim(d_ampcol).or.posfld+nl1.gt.dim(d_ampfld)) then
                    call raiseexception1('ampl',nprt,poscol+nca,posfld+nl1,prcss%col,prcss%flv)
                    mtel=0.d0
                    return
                  endif
!
...
!
!
  integer, parameter :: maxpar=20
  integer, save :: vcnt,tcnt,tbcnt
  character*1 idecay
  common/tdecflag/idecay
  complex*16, dimension(4,2) :: vtp,utpb
  common/tspinors/vtp,utpb
  complex*16 eps(4,maxpar)
  common/vdec/eps
!
  if(prt.eq.1) then; vcnt=0; tcnt=0; tbcnt=0; endif
!
  fld=(0.d0,0.d0)
!
  tmp=prcss%hel(prt)
  ...
  polarization: select case (lortyp(prcss%flv(prt)))
    case (glu)                                      !gluon source
      call sourcemassboson(tmp,tmpc,im,masses(prcss%flv(prt)))
      fld(1:4)=tmpc
      fld(5:10)=0.d0
!      if(prcss%flv(prt+1).eq.1001) fld(1:4)=im
    case (pht)                                      !gluon source
      call sourcemassboson(tmp,tmpc,im,masses(prcss%flv(prt)))
      fld(1:4)=tmpc
      fld(5:10)=0.d0
...
!
  use incnst
  use naming, only : z,wp,wm,photon
  use couplings, only : qrtc,qrtc2,qrtc3,qrtca,qrtchg,gstrong
...
!
  operator: select case (oper)
    case(1)
      call triglu (p1,p2,amp2,p3,amp3,nwam)    !trilinear gluon interaction
    case (4)
      call fbftog (amp2,amp3,nwam)             !fusion of fermions into gluon
!      if(fl1.eq.photon) nwam(5:8)=nwam(11:14)
    case (5)
...
!!$      call gftofr (amp2,amp3,nwam)              !fusion of f-bar  gluon into f-bar
    ...
    case(30)
      call triw (qrtca(1,fl1-1),p1,p2,amp2,p3,amp3,nwam)    !trilinear gluon interaction 
...
!!$      call pptoax2 (qrtc3(1,fl2-1),amp2,amp3,nwam)              !fusion W+ Y++ into W+
    ...
    case (42)
      call aghghtoglu(p2,amp2,amp3,nwam)
    case (43)
      call gluaghtoagh(p3,amp2,amp3,nwam)
    case (44)
      call glughtogh(p1,amp2,amp3,nwam)
    case (45)
...
!
subroutine gluaghtoagh(imp,amp2,amp3,nwam)
!
  use incnst, only : nlor
  implicit none
!
  real*8, intent(in) :: imp(4)
  complex*16, intent(in) :: amp2(nlor),amp3(1)
  complex*16, intent(out) :: nwam(1)
!
  nwam=(imp(1)*amp2(1)-sum(imp(2:4)*amp2(2:4)))*amp3
!
end subroutine gluaghtoagh
!
subroutine glughtogh(imp,amp2,amp3,nwam)
!
  use incnst, only : nlor
  implicit none
!
...
!
  nwam=(-imp(1)*amp2(1)+sum(imp(2:4)*amp2(2:4)))*amp3
!
end subroutine glughtogh
!
subroutine aghghtoglu(imp,amp2,amp3,nwam)
!
  use incnst, only : nlor
  implicit none
!
  real*8, intent(in) :: imp(4)
  complex*16, intent(in) :: amp2(1),amp3(1)
  complex*16, intent(out) :: nwam(nlor)
!
  complex*16 ax(1)
!
  ax(1)=amp2(1)*amp3(1)
  nwam(1)=imp(1)*ax(1); nwam(2:4)=-imp(2:4)*ax(1)
  nwam(5:nlor)=(0.d0,0.d0)
!
end subroutine aghghtoglu
!
...
!
  ...
  complex*16, intent(out) :: nwam(nlor)
!
  real*8, parameter :: cp=-1
!
  nwam(1)=sum(p1(2:4)*(p2(2:4)*amp2(1)-p2(1)*amp2(2:4)+cp*amp2(5:7)))

  nwam(2)=p1(1)*(p2(1)*amp2(2)-p2(2)*amp2(1)-cp*amp2(5))
  nwam(2)=nwam(2)-p1(3)*(p2(3)*amp2(2)-p2(2)*amp2(3)-cp*amp2(10))
  nwam(2)=nwam(2)-p1(4)*(p2(4)*amp2(2)-p2(2)*amp2(4)+cp*amp2(9))

  nwam(3)=p1(1)*(p2(1)*amp2(3)-p2(3)*amp2(1)-cp*amp2(6))
  nwam(3)=nwam(3)-p1(2)*(p2(2)*amp2(3)-p2(3)*amp2(2)+cp*amp2(10))
  nwam(3)=nwam(3)-p1(4)*(p2(4)*amp2(3)-p2(3)*amp2(4)-cp*amp2(8))

  nwam(4)=p1(1)*(p2(1)*amp2(4)-p2(4)*amp2(1)-cp*amp2(7))
  nwam(4)=nwam(4)-p1(2)*(p2(2)*amp2(4)-p2(4)*amp2(2)-cp*amp2(9))
  nwam(4)=nwam(4)-p1(3)*(p2(3)*amp2(4)-p2(4)*amp2(3)+cp*amp2(8))

  nwam(5)=(-p2(1)*amp2(2)+amp2(1)*p2(2))*cp
  nwam(6)=(-p2(1)*amp2(3)+amp2(1)*p2(3))*cp
  nwam(7)=(-p2(1)*amp2(4)+amp2(1)*p2(4))*cp
  nwam(8)=(-p2(3)*amp2(4)+amp2(3)*p2(4))*cp
  nwam(9)=(-p2(4)*amp2(2)+amp2(4)*p2(2))*cp
  nwam(10)=(-p2(2)*amp2(3)+amp2(2)*p2(3))*cp
  nwam(5:7)=nwam(5:7)+amp2(5:7)
  nwam(8:10)=nwam(8:10)-amp2(8:10)

  nwam(1:10)=-nwam(1:10)*amp3(1)
!
!!$  H (dmu anu -dnu amu - xmunu)^2
!!$
!!$  - H (d0 a1 -d1 a0 + x01)^2           x0j 5,6,7
!!$  + H (d1 a2 -d2 a1 + x12)^2           -x23 = 8  
!!$  + H (d3 a1 -d1 a3 + x31)^2           -x31 = 9
!!$  + H (d2 a3 -d3 a2 + x23)^2           -x12 = 10
!!$  
!!$  d / (d a0)   =  -(d1(3)+d1(2)) (d0(2) a1 -d1(2) a0 - x01)^2
!!$                      (=d1(1))
!!$
!!$  d / (d a1)   =  (d0(3)+d0(2)) (d0(2) a1 -d1(2) a0 - x01)^2
!!$                  +(d2(3)+d2(2) (d1 a2 -d2 a1 - x12)
!!$                  -(d3(3)+d3(2) (d3 a1 -d1 a3 - x31)
!
...
!
  real*8, parameter :: cp=-1
!
!  H (p0 A1 - p1 A0 - Y1)^2 - H (p1 A2 - p2 A1 + X3)^2
!
  nwam(1)=-(p2(1)*p3(1)-sum(p2(2:4)*p3(2:4)))*             & 
            (amp2(1)*amp3(1)-sum(amp2(2:4)*amp3(2:4)))
  nwam(1)=nwam(1)+(p2(1)*amp3(1)-sum(p2(2:4)*amp3(2:4)))    &
                    *(amp2(1)*p3(1)-sum(amp2(2:4)*p3(2:4)))
  nwam(1)=nwam(1)+cp*(-(p2(1))*sum( amp2(2:4)*amp3(5:7) )  &
                      -(p3(1))*sum( amp3(2:4)*amp2(5:7) )  &
                      +amp2(1)*sum(p2(2:4)*amp3(5:7))  &
                      +amp3(1)*sum(p3(2:4)*amp2(5:7))  &
                      -(p2(2)*amp2(3)-p2(3)*amp2(2))*amp3(10) &
                      -(p3(2)*amp3(3)-p3(3)*amp3(2))*amp2(10) &
                      -(p2(4)*amp2(2)-p2(2)*amp2(4))*amp3(9) &
                      -(p3(4)*amp3(2)-p3(2)*amp3(4))*amp2(9) &
                      -(p2(3)*amp2(4)-p2(4)*amp2(3))*amp3(8) &
                      -(p3(3)*amp3(4)-p3(4)*amp3(3))*amp2(8) &
                      )
  nwam(1)=(nwam(1)+sum(amp2(5:7)*amp3(5:7))-                 &
                  sum(amp2(8:10)*amp3(8:10)))
  nwam(2:3)=(0.d0,0.d0)
!
...
!
  integer j1
!
  a2a3=sum(amp2(2:4)*amp3(2:4))-amp2(1)*amp3(1)
  a2p=sum(amp2(2:4)*(p3(2:4)-p1(2:4)))-amp2(1)*(p3(1)-p1(1))
  a3p=sum(amp3(2:4)*(p1(2:4)-p2(2:4)))-amp3(1)*(p1(1)-p2(1))
  nwam(1:4)=a2a3*(p2-p3)+a2p*amp3(1:4)+a3p*amp2(1:4); nwam(2:4)=-nwam(2:4)
!  nwam(1:4)=0.d0
! -(a0 a1 - a1 a0) X01 + .... + (a1 a2 - a2 a1) X12
  nwam(1)=nwam(1)-sum(amp2(2:4)*amp3(5:7))+sum(amp3(2:4)*amp2(5:7))
  nwam(2)=nwam(2)+amp3(9)*amp2(4)-amp3(10)*amp2(3)-amp2(9)*amp3(4)+amp2(10)*amp3(3)+amp2(1)*amp3(5)-amp3(1)*amp2(5)
  nwam(3)=nwam(3)+amp3(10)*amp2(2)-amp3(8)*amp2(4)-amp2(10)*amp3(2)+amp2(8)*amp3(4)+amp2(1)*amp3(6)-amp3(1)*amp2(6)
  nwam(4)=nwam(4)+amp3(8)*amp2(3)-amp3(9)*amp2(2) -amp2(8)*amp3(3)+amp2(9)*amp3(2)+amp2(1)*amp3(7)-amp3(1)*amp2(7)
  nwam(5)=-amp2(1)*amp3(2)+amp3(1)*amp2(2)
  nwam(6)=-amp2(1)*amp3(3)+amp3(1)*amp2(3)
  nwam(7)=-amp2(1)*amp3(4)+amp3(1)*amp2(4)
  nwam(8)=-amp2(3)*amp3(4)+amp2(4)*amp3(3)
  nwam(9)=-amp2(4)*amp3(2)+amp2(2)*amp3(4)
  nwam(10)=-amp2(2)*amp3(3)+amp2(3)*amp3(2)
!
end subroutine triglu 
!
subroutine triphw (qrt,p1,p2,amp2,p3,amp3,nwam)   !trilinear WWgamma interaction   
!
  use incnst
  implicit none
!
...
!  nwam(5)=qrt(1)*a2a3
!
!  (wp.wp)(wm.wm)-(wp.wm)(wp.wm) + (wp.wm)
!          [(c z + s ph).(c z +s ph)]-[(c z + s ph).wp]
!          [(c z + s ph). wm]
!
!    ===
!
...
!
  tmp=qrt(1)*amp3(1)+qrt(2)*amp3(2)
  nwam(1:4)=-tmp*amp2(1:4); nwam(1)=-nwam(1)
  nwam(5)=0.d0
!
...
!
  complex*16 :: tmp
!
  tmp=amp2(1)*amp3(1)-sum(amp2(2:4)*amp3(2:4))
  nwam=tmp*qrt
!
...
!
  ...
  complex*16, intent(out), dimension (nlor) :: nwam
!
  nwam(1)=(amp2(1)*amp3(3)+amp2(2)*amp3(4)+amp2(3)*amp3(1)+amp2(4)*amp3(2))
  nwam(2)=-amp2(1)*amp3(4)-amp2(2)*amp3(3)+amp2(3)*amp3(2)+amp2(4)*amp3(1)
  nwam(3)=amp2(1)*amp3(4)-amp2(2)*amp3(3)-amp2(3)*amp3(2)+amp2(4)*amp3(1)
  nwam(3)=(0.d0,1.d0)*nwam(3)
  nwam(4)=-amp2(1)*amp3(3)+amp2(2)*amp3(4)+amp2(3)*amp3(1)-amp2(4)*amp3(2)
  nwam(5:10)=0
!
...
!
  ...
  complex*16, intent(out), dimension (nlor) :: nwam
!
  nwam(1)=amp3(3)*(amp2(4)+amp2(1))+amp3(4)*(amp2(2)+(0.d0,1.d0)*amp2(3))
  nwam(2)=amp3(3)*(amp2(2)-(0.d0,1.d0)*amp2(3))-amp3(4)*(amp2(4)-amp2(1))
  nwam(3)=-amp3(1)*(amp2(4)-amp2(1))-amp3(2)*(amp2(2)+(0.d0,1.d0)*amp2(3))
  nwam(4)=-amp3(1)*(amp2(2)-(0.d0,1.d0)*amp2(3))+amp3(2)*(amp2(4)+amp2(1))
!
end subroutine gfbtofb
!
...
!
  propagator: select case (lortyp(fl))
    case(glu)                                       !massless gauge boson, F_munu
      call gbzms (imp,fus,masses(fl),pol)
    case(pht)                                       !massless gauge boson, F_munu
      call gbzmsph (imp,fus,masses(fl),pol)
    ...
    case(xgl)
      pol(1:6)=fus(1:6)
      write(*,*)'convert to covariant gauge'; stop
    case default
...
!
subroutine gbzmsph (imp,fus,mass,pol)                   !massive gauge boson propagator
!
  use incnst, only : nlor
  use couplings, only : gauge
  implicit none
!
...
!
  ...
  m2=mass*mass; pa=1/(pq-m2*gauge)*(sum(imp*fus(1:4)));
  pq=1.d0/(pq-m2)
  pol(1)=(-fus(1)-(1.d0-gauge)*pa*imp(1))*pq;
  pol(2:4)=(fus(2:4)-(1.d0-gauge)*pa*imp(2:4))*pq;
  pol(5)=fus(5)
!
end subroutine gbzmsph
!
subroutine gbzms (imp,fus,mass,pol)                   !massless gauge boson propagator
!
  use incnst, only : nlor
  use couplings, only : gauge
  implicit none
...
!
  pq=imp(1)*imp(1)-sum(imp(2:4)*imp(2:4))
  m2=mass*mass; pa=1/(pq-gauge*m2)*(sum(imp*fus(1:4)))
  pq=1.d0/(pq-m2)
  pol(1)=(-fus(1)-(1.d0-gauge)*pa*imp(1))*pq;
  pol(2:4)=(fus(2:4)-(1.d0-gauge)*pa*imp(2:4))*pq;
  pol(5:7)=fus(5:7); pol(8:10)=-fus(8:10)
!
...
!
subroutine gbms (imp,fus,mass,width,pol)                   !massive gauge boson propagator
!
  use incnst, only : nlor
  implicit none
!
...
!  pq=imp(1)*imp(1)-sum(imp(2:4)*imp(2:4))-mq             ! +(0.d0,1.d0)*mass*width
  pq=imp(1)*imp(1)-sum(imp(2:4)*imp(2:4))
  theta=1.d0
  if(runwidth.eq.1) theta=pq/mq
  pq=pq-mq             ! +(0.d0,1.d0)*mass*width
  if(wmode.eq.'yy') then
!    pq=1.d0/(pq+(0.d0,1.d0)*mass*width); resonance='n'
    pq=1.d0/(pq+(0.d0,1.d0)*mass*width*theta); resonance='n'
...
!                                                                                         
          data nz/0.d0,0.d0,0.d0,1.d0/                                                            
          data nx/0.d0,1.d0,0.d0,0.d0/                                                            
          data sour1/4*0.d0/                                                                
          data sour2/4*0.d0/                                                                
          data sour3/4*0.d0/                                                                
!                                                                                         
...
!*****************************************************
!*****************************************************
!!$subroutine triglu_old (p1,p2,amp2,p3,amp3,nwam)     !trilinear gluon interaction   !
!!$  use incnst
!!$  implicit none
!!$!
!!$  complex*16, intent(in), dimension (nlor) :: amp2,amp3
...
!!$!
!!$end subroutine triglu_old
!!$!

!!$
!!$subroutine gbzms_old (imp,fus,pol)                   !massless gauge boson propagator
!!$!
!!$  use incnst, only : nlor
!!$  implicit none
!!$!
...
!!$  write(*,*)'must be converted in covariant gauge'; stop
!!$!
...
!!$  complex*16, intent(out) :: nwam(nlor)
!!$!
!!$  nwam(2)=-p2(3)*amp2(4)+p2(4)*amp2(3)
!!$  nwam(3)=-p2(4)*amp2(2)+p2(2)*amp2(4)
!!$  nwam(4)=-p2(2)*amp2(3)+p2(3)*amp2(2)
!!$
!!$  nwam(1:3)=nwam(1:3)*amp3(1)
!!$!
