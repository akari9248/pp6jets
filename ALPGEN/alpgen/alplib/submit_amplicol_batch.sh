...
mkdir -p "${SCRIPT_DIR}/AmpliCol/log"

EOS_BASE="/eos/cms/store/group/phys_smp/ec/zhye/Amplicol/pp6j_20GeV_SC2"

for PART_NUM in $(seq $START $END); do
    echo "[Part${PART_NUM}] Creating EOS directory..."
    eos root://eoscms.cern.ch mkdir -p "${EOS_BASE}/Part${PART_NUM}" 2>/dev/null
