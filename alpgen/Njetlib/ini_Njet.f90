...
!!$                      xpp,wm,wm,ypp,wp,wp,gluon,gluon,gluon,higgs,higgs,higgs /) 
  tabint(1:31,1)=(/10,qupl,qupbrl,gluon, qupr,qupbrr,gluon, qdr,qdbrr,gluon, qdl,qdbrl,gluon,  &
                      qcr,qcbrr,gluon, qcl,qcbrl,gluon, qsr,qsbrr,gluon, qsl,qsbrl,gluon,      &
                     gluon,gluon,gluon, qbt,qbtbr,gluon/)
...
!
  ...
  masses(qbt)= 0.d0;   masses(qbtbr)= masses(qbt);  masses(qtop)=apar(16); 
...
!
  chdim : select case (npr)
    case (4)
      dim(d_offsh)=110; dim(d_offshcol)=40; dim(d_ampfld)=50; dim(d_ampcol)=20
    case (5)
      dim(d_offsh)=190; dim(d_offshcol)=60; dim(d_ampfld)=90; dim(d_ampcol)=30
    case (6)
      dim(d_offsh)=370; dim(d_offshcol)=110; dim(d_ampfld)=210; dim(d_ampcol)=70
    case (7)
      dim(d_offsh)=710; dim(d_offshcol)=220; dim(d_ampfld)=390; dim(d_ampcol)=120
    case (8)
      dim(d_offsh)=1390; dim(d_offshcol)=420; dim(d_ampfld)=730; dim(d_ampcol)=220
    case (9)
      dim(d_offsh)=2710; dim(d_offshcol)=820; dim(d_ampfld)=1390; dim(d_ampcol)=420
    case (10)
      dim(d_offsh)=5390; dim(d_offshcol)=1620; dim(d_ampfld)=2690; dim(d_ampcol)=810
    case (11)
      dim(d_offsh)=10670; dim(d_offshcol)=3200; dim(d_ampfld)=5270; dim(d_ampcol)=1580
    case (12)
      dim(d_offsh)=21500; dim(d_offshcol)=6500; dim(d_ampfld)=10000; dim(d_ampcol)=3000
    case (13)
      dim(d_offsh)=42500; dim(d_offshcol)=13000; dim(d_ampfld)=20000; dim(d_ampcol)=6000
    case (14)
      dim(d_offsh)=86500; dim(d_offshcol)=26500; dim(d_ampfld)=39500; dim(d_ampcol)=12000
    case (15)
      dim(d_offsh)=240000; dim(d_offshcol)=120000; dim(d_ampfld)=160000; dim(d_ampcol)=60000
    case (16)
      dim(d_offsh)=480000; dim(d_offshcol)=240000; dim(d_ampfld)=320000; dim(d_ampcol)=120000
    case (17)
      dim(d_offsh)=1000000; dim(d_offshcol)=500000; dim(d_ampfld)=640000; dim(d_ampcol)=300000
    case (18)
      dim(d_offsh)=2000000; dim(d_offshcol)=1000000; dim(d_ampfld)=1280000; dim(d_ampcol)=480000
    case (19)
      dim(d_offsh)=4000000; dim(d_offshcol)=2000000; dim(d_ampfld)=2300000; dim(d_ampcol)=1000000
    case (20)
      dim(d_offsh)=8000000; dim(d_offshcol)=4000000; dim(d_ampfld)=4600000; dim(d_ampcol)=2000000
